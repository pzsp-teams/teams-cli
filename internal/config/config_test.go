...
func TestFindDefaultConfigFile(t *testing.T) {
	t.Run("no config file exists", func(t *testing.T) {
		...
		got := FindDefaultConfigFile()
		if got != "" {
			t.Errorf("FindDefaultConfigFile() = %q, want empty string", got)
		}
	})
	...
	t.Run("config file exists", func(t *testing.T) {
		...
		configPath := filepath.Join(tmpDir, "teams_cli.json")
		if err := os.WriteFile(configPath, []byte("{}"), 0o644); err != nil {
			t.Fatal(err)
		}

		got := FindDefaultConfigFile()
		if got != "teams_cli.json" {
			t.Errorf("FindDefaultConfigFile() = %q, want %q", got, "teams_cli.json")
		}
	})

	t.Run("multiple config files exist, returns first", func(t *testing.T) {
		// Create a temporary directory
		tmpDir := t.TempDir()
		...
		for _, filename := range []string{"teams_cli.toml", "teams_cli.json", "teams_cli.yaml"} {
		...
		got := FindDefaultConfigFile()
		if got != "teams_cli.toml" {
			t.Errorf("FindDefaultConfigFile() = %q, want %q", got, "teams_cli.toml")
		}
	})
}
